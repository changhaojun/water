...
var systemName="water";
//获取账户列表
$(function(){
	searchFilter='{"customer_id":"'+customerId+'","company_id":"'+companyId+'"}';
	getToken();
	allList();
	showTips();
	setRole();
...
function chooseCompany(){
		$.ajax({
			type:"get",
			url:manage_globalurl+"/v1/companies",
			async:true,
			dataType:'JSON',
			data:{
...
function addUser(){
	...
	if ($("#addText").val() == "请输入邮箱") {
		...
		var data = "{'company_id':'"+companyId+"','system':"+JSON.stringify(addSystem)+",'fullname':'" + $("#addUsername").val() + "','username':'" + $("#addText").val() + "','password':'" + $("#addPassword").val() + "','mobile':'" + $("#addPhone").val() + "','status':" + flag + ",'customer_id':'" + customerId + "'}";
...
function modify(_id){
	...
	$(".modifySystem ul li").find("input").attr({"checked":false});
	$.ajax({
		type: 'get',
		crossDomain: true == !(document.all),
		url: manage_globalurl + "/v1/users/",
...
function sureModify() {
	...
	if ($("#modifyText").val() == "请输入邮箱") {
		...
		var modifydata = "{'company_id':'"+companyId+"','system':"+addSystem+",'fullname':'" + $("#modifyUsername").val() + "','username':'" + $("#modifyText").val() + "','password':'" + $("#modifyPassword").val() + "','mobile':'" + $("#modifyPhone").val() + "','status':" + setVar + "}";
		$.ajax({
			type: 'put',
			url: manage_globalurl + "/v1/users/" +editId,
			dataType: "JSON",
			crossDomain: true == !(document.all),
			data: {
				roles: addoRole,
...
function sureDel() {
	$.ajax({
		type: 'delete',
		crossDomain: true == !(document.all),
		url: manage_globalurl + "/v1/users/" + editId + "?access_token=" + accesstoken,
		dataType: 'json',
		data:{
			filter:searchFilter
		},
...
$(".closeAdd").on("click", function() {
		...
		$(".addSystem ul li").find("input").attr({"checked":false});
})
//按回车键进行查找
